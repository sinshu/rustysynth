...
impl Voice
{
    ...
    pub(crate) fn start(&mut self, data: &Rc<Vec<i16>>, region: &RegionPair, channel: i32, key: i32, velocity: i32)
    {
        ...
        if velocity > 0
        {
            ...
            let decibels = 2_f32 * SoundFontMath::linear_to_decibels(velocity as f32 / 127_f32) - sample_attenuation - filter_attenuation;
    ...
    pub(crate) fn process(&mut self, channels: &Vec<Channel>) -> bool
    {
        ...
        let vib_pitch_change = (0.01_f32 * channel_info.get_modulation() + self.vib_lfo_to_pitch) * self.vib_lfo.get_value();
        let mod_pitch_change = self.mod_lfo_to_pitch * self.mod_lfo.get_value() + self.mod_env_to_pitch * self.mod_env.get_value();
        ...
        let angle = (consts::PI / 200_f32) * (channel_info.get_pan() + self.instrument_pan + 50_f32);
        if angle <= 0_f32
        {
        ...
        self.current_reverb_send = SoundFontMath::clamp(channel_info.get_reverb_send() + self.instrument_reverb, 0_f32, 1_f32);
        self.current_chorus_send = SoundFontMath::clamp(channel_info.get_chorus_send() + self.instrument_chorus, 0_f32, 1_f32);
