...
impl VolumeEnvelope
{
    ...
    pub(crate) fn process(&mut self, sample_count: i32) -> bool
    {
        ...
        {
            self.value = SoundFontMath::max(SoundFontMath::exp_cutoff(self.decay_slope * (current_time - self.decay_start_time)) as f32, self.sustain_level);
            self.priority = 1_f32 + self.value;
            return self.value > SoundFontMath::NON_AUDIBLE;
        }
        else if self.stage == EnvelopeStage::RELEASE
        {
            self.value = (self.release_level as f64 * SoundFontMath::exp_cutoff(self.release_slope * (current_time - self.release_start_time))) as f32;
