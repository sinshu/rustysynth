...
impl BinaryReader
{
    ...
    pub(crate) fn read_i32_variable_length<R: Read>(reader: &mut R) -> Result<i32, Box<dyn Error>>
    {
        ...
        loop
        {
            ...
            if count == 4
            {
                return Err(format!("The length of the value must be equal to or less than 4.").into());
            }
        }
    ...
    pub(crate) fn read_fixed_length_string<R: Read>(reader: &mut R, length: i32) -> Result<String, Box<dyn Error>>
    {
    ...
    pub(crate) fn read_wave_data<R: Read>(reader: &mut R, size: i32) -> Result<Vec<i16>, io::Error>
    {
