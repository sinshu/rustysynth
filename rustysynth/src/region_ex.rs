...
impl RegionEx
{
    ...
    pub(crate) fn start_volume_envelope(envelope: &mut VolumeEnvelope, region: &RegionPair, key: i32, _velocity: i32)
    {
        ...
        let hold = region.get_hold_volume_envelope() * SoundFontMath::key_number_to_multiplying_factor(region.get_key_number_to_volume_envelope_hold(), key);
        let decay = region.get_decay_volume_envelope() * SoundFontMath::key_number_to_multiplying_factor(region.get_key_number_to_volume_envelope_decay(), key);
    ...
    pub(crate) fn start_modulation_envelope(envelope: &mut ModulationEnvelope, region: &RegionPair, key: i32, velocity: i32)
    {
         ...
         let hold = region.get_hold_modulation_envelope() * SoundFontMath::key_number_to_multiplying_factor(region.get_key_number_to_modulation_envelope_hold(), key);
         let decay = region.get_decay_modulation_envelope() * SoundFontMath::key_number_to_multiplying_factor(region.get_key_number_to_modulation_envelope_decay(), key);
    ...
    {
        lfo.start(region.get_delay_vibrato_lfo(), region.get_frequency_vibrato_lfo());
    }

    pub(crate) fn start_modulation(lfo: &mut Lfo, region: &RegionPair, _key: i32, _velocity: i32)
    {
        lfo.start(region.get_delay_modulation_lfo(), region.get_frequency_modulation_lfo());
    }
}
